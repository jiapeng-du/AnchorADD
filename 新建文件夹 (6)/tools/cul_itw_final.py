...
def plot_reliability_diagram_v2(labels, probs, n_bins=10, save_path='reliability_diagram.png', ece=None):
    ...
    ax.bar(bin_centers, accuracies, width=bar_width, color=colors, edgecolor='black', linewidth=0.7)
    ...
    for i, count in enumerate(bin_counts):
        if not np.isnan(accuracies[i]):
            ax.text(bin_centers[i], accuracies[i] + 0.03, f'{count}', ha='center',
...
def compute_calibration_metrics(cm_scores, invert=False, plot=True, output_prefix=""):
    ...
    - Brier Score
    - ECE
    - ✅ AUROC(uncorr): “错判=1 / 对判=0” vs 不确定性(熵) 的 AUROC（与 cul-eer21 一致）
    """
    # 提取分数和标签
    raw_scores = cm_scores['1_x'].values
    labels = (cm_scores[4] == 'bonafide').astype(int).values  # bonafide=1, spoof=0
    ...
    # ===== ✅ AUROC(uncorr)（和 cul-eer21 一样的定义）=====
    preds = (probs >= 0.5).astype(int)
    correct = (preds == labels).astype(int)
    ...
    if (y_binary.sum() == 0) or (y_binary.sum() == y_binary.size):
        print("WARNING: All predictions are either correct-only or wrong-only; cannot compute uncorr AUROC.")
        auroc = float('nan')
    else:
...
def eval_to_score_file(score_file, cm_key_file, protocol_file):
    # 加载eval trials
    ...
    cm_scores = submission_scores.merge(cm_data_eval, left_on=0, right_on=1, how='inner')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--protocol_file', type=str,
                        default='/root/autodl-tmp/datasets/release_in_the_wild/protocols/in_the_wild/in_the_wild.eval.trl.txt')
    args = parser.parse_args()

    cm_key_file = "/root/autodl-tmp/datasets/release_in_the_wild/meta.txt"
    ...
    eval_to_score_file(args.scoreFile, cm_key_file, args.protocol_file)
